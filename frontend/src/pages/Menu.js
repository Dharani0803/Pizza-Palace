...
function Menu(){
  ...
  .filter((pizza) => {
    const categoryMatch = pizza.category === selectedCategory;
    const searchMatch = pizza.name
      .toLowerCase()
      .includes(searchTerm.toLowerCase());
    const vegMatch = vegFilter ? pizza.isVeg : true;
    const nonVegMatch = nonVegFilter ? !pizza.isVeg : true;
    return (
      categoryMatch &&
      searchMatch &&
      vegMatch &&
...
>
      <nav className="flex justify-between items-center m-3">
        ...
        <div>
          <i onClick={() => setShowSearch(!showSearch)} className="fa-solid fa-magnifying-glass bg-[#FFFFFF] rounded-full border border-gray-400 p-2 cursor-pointer"></i>
        </div>
      </nav>

    {showSearch && (
    <div className="px-3 mt-4">
      <input type="text" placeholder="Search pizzas..." value={searchTerm}
       onChange={(e) => setSearchTerm(e.target.value)} className="w-full border border-gray-300 rounded-lg px-4 py-2 outline-none"/>
    </div>)}
    ...
    <div className="flex mt-5 overflow-x-auto  whitespace-nowrap [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      ...
      ${ selectedCategory === category ? "bg-red-600 text-white" : "bg-gray-200 text-black"}`}
