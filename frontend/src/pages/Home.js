...
if (loading) {
  return(
    ...
    <div className="relative">
      ...
      <button className="w-full text-left px-4 py-4 hover:bg-gray-100 text-sm">
    ...
    <form className="space-y-4">
      ...
      <input type="text" placeholder="City" value={city} onChange={(e) => setCity(e.target.value)}
       className="w-full border border-gray-300 rounded-lg px-4 py-3 outline-none"/>
      <button onClick={() => { const fullAddress = `${house}, ${street}, ${landmark}, ${city}`;
  ...
  {bestsellers.map((item, index) => {const cartItem = cartItems.find((cart) => cart._id === item._id);

  return (
    <div onClick={() => navigate(`/pizza/${item._id}`, { state: item })} key={index}
    ...
    <div className="flex items-center p-3 shadow-[0_-4px_10px_rgba(0,0,0,0.15)] justify-between border-t inset-0 bg-black/30 backdrop-blur-md">
      <div className="flex-col">
        ...
        <p className="text-xs font-bold border-b pb-1 border-dashed">
          Regular | New Hand Tossed<i className="fa-solid fa-angle-right ml-1"></i></p>
      </div>
  ...
  <div className="absolute bottom-0 text-white w-full">
    ...
    <div className="flex items-center p-3 justify-between border border-t-1 inset-0 bg-black/30 backdrop-blur-md">
      <div className="flex-col">
        <span className="text-xl font-bold">₹{item.price}</span>
        <p className="text-xs font-bold border-b pb-1 border-dashed">Regular Plus | Cheese Volcano <i class="fa-solid fa-angle-right"></i></p>
      </div>

    {cartItems.find((cart) => cart._id === item._id) ? (
    <div className="flex items-center gap-4 bg-transparent border border-gray-200 text-white px-4 py-2 rounded-md">
      <button onClick={() => decreaseQuantity(item._id)}> - </button>
      <span>{ cartItems.find((cart) => cart._id === item._id).quantity }</span>
      <button onClick={() => increaseQuantity(item._id)}> + </button>
    </div>) : (
    ...
    <div className="flex-col">
      <span className="text-xl font-bold">₹{item.price}</span>
      <p className="text-xs font-bold border-b pb-1 border-dashed">Regular | New Hand Tossed <i class="fa-solid fa-angle-right"></i></p>
    </div>
