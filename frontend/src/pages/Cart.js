...
function Cart(){
    ...
    const { cartItems, increaseQuantity, decreaseQuantity, appliedOffer, setAppliedOffer } = useContext(CartContext);
    ...
    return(
      <div>
        ...
        <div className={`flex gap-1 items-center px-2 py-1 rounded-md ${ appliedOffer ? "bg-green-100 text-green-700" : "text-gray-700" }`}>
          <i className={`fa-solid ${appliedOffer ? "fa-circle-check" : "fa-tag"}`}></i>
          <p className="text-sm font-semibold">{appliedOffer ? "Offer Applied" : "Offers"}</p>
        </div>
        </nav>
      ...
      <div className="bg-white p-3 flex gap-5 items-center rounded-lg mb-5">
        ...
        <div className="bg-[#FFFFFF] p-2 border border-gray-100 rounded-lg flex gap-1 items-center cursor-pointer">
          <div>
            <p className="text-gray-500 text-xs font-semibold">DELIVER</p>
            <p className="text-black font-bold text-lg">Now</p>
          </div>
          <div><i class="fa-solid fa-angle-down text-gray-500 text-xs"></i></div>
        </div>

        <div>
        <div className="flex items-center gap-1">
          <p className="text-xs font-bold">Home</p>
          <i class="fa-solid fa-angle-down text-gray-500 text-xs"></i>
        </div>
        <div>
          <p className="text-gray-500 text-xs max-w-[220px] truncate">{address || "Add Address"}</p></div>
        </div>
      </div>
