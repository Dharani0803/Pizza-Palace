...
<div className="fixed top-0 left-0 h-full w-[320px] bg-white shadow-2xl z-50 overflow-y-auto">
  ...
  onChange={(e) => setFullName(e.target.value)} className="w-full border border-gray-300 rounded-lg px-4 py-3 outline-none"/>
      <input type="text" placeholder="Mobile Number"value={mobile}
  onChange={(e) => setMobile(e.target.value)}
 className="w-full border border-gray-300 rounded-lg px-4 py-3 outline-none"/>
      <input type="text" placeholder="Flat / House No"  value={house}
  onChange={(e) => setHouse(e.target.value)}
  className="w-full border border-gray-300 rounded-lg px-4 py-3 outline-none"/>
      <input type="text" placeholder="Street / Area" value={street}
 ...
 className="w-full border border-gray-300 rounded-lg px-4 py-3 outline-none"/>
  ...
  onChange={(e) => setCity(e.target.value)} className="w-full border border-gray-300 rounded-lg px-4 py-3 outline-none"/>
      <button
  onClick={() => {
    const fullAddress =
 ...
 className="w-full bg-[#E31837] text-white py-3 rounded-lg font-semibold">Save Address</button>
      </form>
    </div></div>)}
    ...
    <div className="m-5">
      <h1 className="text-xl font-bold">Offers for you</h1>
        <div className="flex flex-col md:flex-row mt-3 md:gap-10 gap-3">
          ...
          <div className="flex gap-10">
            <div className="flex gap-40 items-center rounded-lg text-white p-3 bg-gradient-to-b from-orange-700 to-orange-500">
...
<div onClick={() =>
  navigate(`/pizza/${item._id}`, { state: item })
}
  key={index}
...
>
  ...
  <div className="flex gap-5 overflow-x-auto scroll-smooth [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
    ...
    {CheeseVolcano.map((item, index) => (
      ...
      <div key={index} className="relative min-w-[340px] h-[350px] rounded-xl overflow-hidden flex-shrink-0">
        ...
        <h1 className="text-lg font-bold">{item.name}</h1></div>
        <p className="text-xs text-gray-200 line-clamp-2">{item.description}</p></div>

        <div className="flex items-center p-3 justify-between border border-t-1 inset-0 bg-black/30 backdrop-blur-md">
        <div className="flex-col">
        <span className="text-xl font-bold">₹{item.price}</span>
        <p className="text-xs font-bold border-b pb-1 border-dashed">Regular Plus | Cheese Volcano <i class="fa-solid fa-angle-right"></i></p></div>
        {cartItems.find((cart) => cart._id === item._id) ? (
  ...
  <div className="flex items-center gap-4 bg-transparent border border-gray-200 text-white px-4 py-2 rounded-md">
    ...
    <span>
      {
        cartItems.find((cart) => cart._id === item._id).quantity
      }
    </span>
  ...
  <button
    onClick={() => handleAddToCart(item)}
    className="bg-[#E31837] px-5 py-2 rounded-md font-semibold"
  >
    Add +
  </button>
  ...
  <div className="flex gap-3 pl-2 mt-5 items-center">
    <div><img className="w-16" src={Lunch} alt="Lunch Feast"></img></div>
      <div><p className="font-semibold text-2xl">Lunch Feast @ ₹149</p>
    ...
    {LunchFeast.map((item, index) => (

      <div key={index} className="relative min-w-[340px] h-[350px] rounded-xl overflow-hidden flex-shrink-0">
      <img src={item.imageUrl} alt="Pizza" className="w-full h-full object-cover"/>
